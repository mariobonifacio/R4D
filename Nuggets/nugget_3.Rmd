...
str(flightcarrier)
print('Yes, the flightcarrier vector is a chr. vector')
```
...
```{r}
pulledCarrier <- pull(.data = flights, var = carrier) %>%
  unique()
str(pulledCarrier)
```
...
```{r}
print('select() returns a list while pull() returns a vector')
```
...
flights %>%
  select(1:5)
flights[,1:5]
cbind(pull(flights,'year'),pull(flights,'month'),pull(flights,'day'),pull(flights,'dep_time'),pull(flights,'sched_dep_time'))
```
...
origin_countdelays <- data.frame(origin_apt = c("EWR","JFK","LGA"),countdelays = c(nrow(filter(flights,origin=='EWR' & dep_delay>10)),nrow(filter(flights,origin=='JFK' & dep_delay>10)),nrow(filter(flights,origin=='LGA' & dep_delay>10))));

origin_countdelays

arrange(origin_countdelays,desc(countdelays)); print ("EWR had the most delays < 10 min")
...
It's fine. All carriers seem to have similar histograms
