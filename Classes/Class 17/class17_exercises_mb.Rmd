...
```{r}
my_x <- vector(mode = 'numeric', length = 20)
...
```{r}
my_list <- vector(mode = 'list', length = length(x))

## for(i in 1:10){my_temp <- gapminder[[,i]]
##    my_list[i] <- if_else(is.numeric(my_temp)##
#                       ,mean(my_temp, na.rm = T)
#                       ,for(j in seq_along(my_temp)){}  #stumped here paste(my_temp[[seq_along(temp)]])}
```
...
```{r}
mt_cols <- seq_along(colnames(gapminder))
```
