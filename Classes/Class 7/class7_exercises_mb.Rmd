...
gapminder %>% 
    ...
    filter(year == max(year, na.rm = T)) %>% 
    ungroup() %>% 
    ggplot(data = ., mapping = aes(x = log(pop), y = log(gdpPercap))) +
    geom_point()
```
...
gapminder %>% 
    ...
    ggplot(data = ., mapping = aes(x = log(pop), y = log(gdpPercap))) +
    geom_point() +
  geom_smooth(method='lm',fill='red',mapping = aes(alpha = 1))
...
gapminder %>% 
    ...
    filter(year == max(year, na.rm = T)) %>% 
    ungroup() %>% 
    ggplot(data = ., mapping = aes(x = log(pop), y = log(gdpPercap),color=continent)) +
    geom_point() +
  geom_smooth(method='lm',fill='red',mapping = aes(alpha = 1))
...
gapminder %>%
    ...
    summarise(mean_GDPpercap = mean(gdpPercap, na.rm = T)) %>%
    ungroup() %>% 
    ggplot(mapping = aes(x = year, y = mean_GDPpercap, colour = continent)) +
    geom_line(na.rm = T) +
    geom_point(alpha = 0.5, size = 2.5, na.rm = T) +
    labs(
        x = "year",
        y = "mean gdp (USD)",
        title = 'Oceania has just 2 countries'
        ) +
    theme_minimal()
```
...
library(gapminder);library(nycflights13);library(tidyverse)

gapminder %>%
  group_by(country) %>%
  mutate(mean_hist_gdppercap = mean(gdpPercap, na.rm = T)) %>%  
  ungroup() %>%
  select(year,mean_hist_gdppercap) %>%
  mutate(incomeGroup = cut_number(mean_hist_gdppercap,5)) %>%
  group_by(year) %>%
  group_by(incomeGroup) %>%
  arrange(year,incomeGroup) %>%
  mutate(mean_of_group=mean(mean_hist_gdppercap,na.rm = T)) %>%
  ungroup() %>%
  ungroup() %>%
  select(year,mean_of_group,incomeGroup) %>%
  unique() %>%
  ggplot(mapping = aes(x = year,y=mean_of_group,color = incomeGroup)) +
  geom_line()
