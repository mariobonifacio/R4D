...
gapminder %>%
  ...
  mutate(chg_lifeexp_pct = 100 * (lifeExp - lag(lifeExp)) / lag(lifeExp)) %>%
  ungroup()

chg <- function(x) 100 * (x - lag(x)) / lag(x)
...
plot_data <- plot_data %>%
  group_by(country) %>%
  mutate(sharp = if_else(min(chg_lifeexp_pct, na.rm = T) < -15, 'Sharp Decline', 'stable')) %>%
  ungroup()
```
...
```{r}
plot_data %>%
  ggplot(mapping = aes(x = year, y = lifeExp, group = country, color = sharp, na.rm = T)) +
  geom_line()
```
...
plot_data %>%
  ggplot(mapping = aes(x = year, y = lifeExp, group = country, color = sharp, na.rm = T)) +
  geom_line() +
  scale_alpha_manual("Sharp Decline" = )
...
```{r}
plot_data %>%
  ggplot(mapping = aes(x = year, y = lifeExp, group = country, alpha = sharp, color = sharp, na.rm = T)) +
  geom_line() +
  scale_alpha_manual(values = c("Sharp Decline" = 1, "stable" = 0.3)) +
  scale_color_manual(values = c("Sharp Decline" = "red", "stable" = "gray"))
