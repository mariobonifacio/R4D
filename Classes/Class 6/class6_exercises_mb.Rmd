...
It's already been installed. Might accidentally install it again and overwrite something
...
GDP per capita has a very long tail whereas life expectancy does not.
...
my_gap %>%
  group_by(year) %>%
  mutate(mean_pop = mean(pop, na.rm = T)) %>%
  select(year,mean_pop) %>%
  ungroup() %>%
  unique() %>%
