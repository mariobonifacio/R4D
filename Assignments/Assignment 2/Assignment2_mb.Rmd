...
**HYPOTHESIS:** My null hypothesis is that *no* available weather or flight data shares a relationship with flight delay. I'll seek to disprove this.
...
### Join flight, plane, and weather data into a data frame of raw data
flt_raw <- flights %>%
  mutate(dep_delay = (abs(dep_delay) + dep_delay) / 2) %>%
  left_join(rename(planes,'manuf_year' = year), by = 'tailnum') %>%
...
mean_cat <- function(CAT_COL, new_col_name) {group_by(flt_test, flt_test[,CAT_COL]) 
  cat_facts <- rbind(cat_facts, cbind(cat = colnames(flt_test[,CAT_COL]), sub_cat = flt_test[,CAT_COL]
            ,fact = flt_test %>%
               select(dep_delay, flt_test[,CAT_COL]) %>%
               group_by(flt_test[,CAT_COL]) %>%
               mutate(temp = mean(dep_delay, na.rm = T)) %>%
               ungroup() %>%
               select(temp)))
  flt_test[,CAT_COL] <- mean(dep_delay, na.rm = T)
  colnames(flt_test[,CAT_COL]) <- new_col_name
  ungroup(flt_test)}
