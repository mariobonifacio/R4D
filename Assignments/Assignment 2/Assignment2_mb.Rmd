...
**HYPOTHESIS:** I'll hypothesize that these data 
...
flt_wx <- flights %>%
  ...
  ungroup() %>%
  group_by(carrier) %>%
  mutate(carr_effect = mean(dep_delay,na.rm = T)) %>%
  ungroup() %>%
  group_by(wkday) %>%
  mutate(wkday_effect = mean(dep_delay,na.rm = T)) %>%
  ungroup() %>%
  mutate(carr_effect = carr_effect - mean(carr_effect,na.rm=T)
        ,wkday_effect = wkday_effect - mean(wkday_effect,na.rm=T)) %>%
...
flights %>%
  ...
  cor(use = 'pairwise.complete.obs',method = 'pearson')
  ggplot(data = .,mapping = aes(x=hour,y=mean_delay)) +
  geom_line()
