...
```{r echo=FALSE}
library(tidyverse); library(nycflights13); library(stringr); library(lubridate) ## Loading  these

test_a <- flights %>% ### 
  filter(!is.na(dep_delay) == T) %>% 
  group_by(sched_dep_time) %>%
  filter(n() >= 100) %>%
  mutate(colon_time = hm(paste(trunc(sched_dep_time/100)
                            ,str_sub(sched_dep_time,-2,-1)
                            ,sep = ':')))
