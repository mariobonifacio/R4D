...
> Probability of departure delay = 1 / (1 + e^-(0.27 - 0.0012 * difference between 8pm and scheduled departure time in minutes))

```{r echo=FALSE, message=FALSE, warning=FALSE}
## Load needed packages
library(tidyverse); library(nycflights13); library(stringr); library(lubridate); library(ggthemes)
```
...
test_c <- test_b %>%
  mutate(is_delay = if_else(dep_delay > 0, 1, 0)
         ,diff_20h = abs(as.numeric(dep_time_object - hm('20:00')) / 60)) %>%
  group_by(diff_20h) %>%
  mutate(op_diff20 = mean(is_delay, na.rm = T)) %>%
  ungroup()
...
test_d <- test_c %>%
  mutate(pred_bivar = 1/(1+exp(-(0.2746527 - 0.001169423 * diff_20h))))
```

```{r, echo=FALSE, warning=F}

ggplot(data = test_d, mapping = aes(x = diff_20h, y = op_diff20, color = 'observed probability')) +
  geom_point(size = 1) +
  geom_line(data = test_d, mapping = aes(x = diff_20h, y = pred_bivar, color = 'predicted probability'), size = 1.5) +
  geom_hline(yintercept = 0.39094, size = 1, aes(color = 'gray')) +
  ggtitle('Fly in the morning to lower your chance for delay') +
  scale_color_fivethirtyeight(labels = c('observed probability', 'predicted probability', 'baseline prediction')
                              ,name = 'Time diff. of scheduled departure and 8 p.m.') +
...
Some flight, plane, and weather data are relevant or redundant and may be removed
```{r}
### Remove irrelevant columns
...
flt_test_01 %>%
  ...
  filter(n() > 100, sched_dep_time > 1199) %>%
  mutate(op_sched_dep = mean(is_delay)
...
temp_df <- flights %>%
  ...
  select(sched_dep_time_metric, is_delay)

results_df <- tibble(i = 0L, cor_i = 0.1234)

for(i in 1:2399){
  results_df[i, 1] <- i
  results_df[i, 2] <- cor(abs(i - temp_df$sched_dep_time_metric), temp_df$is_delay)
}

results_df %>% arrange(desc(abs(cor_i)))

summary(lm(data = temp_df, formula = is_delay ~ sched_dep_time_metric))

ggplot(results_df, aes(i, cor_i, color = 'corr')) +
  geom_line() +
  theme_fivethirtyeight() +
  scale_color_fivethirtyeight() +
geom_line(aes(i, abs(cor_i)/max(abs(cor_i)), color = 'abs_corr'))
  
```

This graph appears to show that the probability of departure delay increases as the day moves along, peaks around 8 p.m., then drops again until midnight.

Based on the assumption that the delays are approximately symmetric about 8 p.m., I can mutate scheduled departure time by finding the time difference between each scheduled flight time and 8 p.m.
```{r, message=FALSE, warning=FALSE, include=FALSE}
flt_test_02 <- flt_test_01 %>%
  mutate(is_delay = if_else(dep_delay > 0, 1, 0)
         ,diff_20h = abs(as.numeric(sched_dep_time_object - hm('20:00')) / 60)) %>%
  group_by(diff_20h) %>%
  mutate(op_diff20 = mean(is_delay, na.rm = T)) %>%
  ungroup()
```
...
Because travelers may want to refer to delays associated with members of each category, the observed probabilities will be listed in the Appendix alongside their names in a table.
```{r, message=FALSE, warning=FALSE, include=FALSE}
...
flights %>% ##
  ...
  as_tibble() %>%
  ggplot(data = ., mapping = aes(x = dep_delay, fill = 'departure delay (min)')) +
...
```
Let's look at 6 percentiles to verify that this is not normal by checking the probabilities associated with one-tailed z-scores of 0.5 to 3.0, by 0.5. If this were a normal distribution, these 
```{r, message=FALSE, warning=FALSE, include=FALSE}
...
```{r, message=FALSE, warning=FALSE}
glm_test <- glm(data = flt_test_03, is_delay ~ distance + manuf_year + engines + seats + speed + month.y + temp + dewp + humid + wind_dir + wind_speed + wind_gust + precip + pressure + visib + diff_20h + op_carr + op_ogn + op_type + op_mnf + op_mdl + op_spd + op_eng, family = binomial)

summary(glm_test)

glm_test <- glm(data = flt_test_03, is_delay ~ month.y + temp + humid + wind_gust + diff_20h + op_carr + op_mdl, family = binomial)
...
There are updates to the flight data for as recently as 2017. Let's see how the formula did at predicting. You may need to download this file and store it in the directory you have this notebook: http://mariobonifacio.github.io/flights17.rds
...
# Then put that file in this directory
library(RCurl)
#
f <- CFILE("flights17.rds", mode="wb")
curlPerform(url = "http://mariobonifacio.github.io/flights17.rds", writedata = f@ref, noprogress=FALSE) # If this line doesn't work, download and put in the directory of this notebook
close(f)
...
flights_17 %>%
  ...
  theme_fivethirtyeight() +
  scale_color_fivethirtyeight() +
  ggtitle('My bivariate logistic regression model works well on 2017 data')
