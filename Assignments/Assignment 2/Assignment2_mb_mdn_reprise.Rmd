...
test_c <- test_b %>%
  mutate(is_delay = if_else(dep_delay > 0, 1, 0)
         ,diff_20h = abs(as.numeric(dep_time_object - hm('20:00'), units = 'minutes'))) %>%
  group_by(diff_20h) %>%
...
test_d <- test_c %>%
  ...
  select(diff_20h, is_delay, pred_bivar, op_diff20, dep_time_object, sched_dep_time, dep_delay)
...
ggplot(data = test_d, mapping = aes(x = diff_20h, y = op_diff20, color = 'observed probability')) +
  ...
  geom_hline(yintercept = 0.39094, size = 1) +
