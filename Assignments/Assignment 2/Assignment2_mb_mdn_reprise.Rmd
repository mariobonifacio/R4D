...
for(i in 1:2399){results_df[i, 1] <- paste(trunc(i / 100), trunc((i %% 100) * 3 / 5), sep = ':')
  results_df[i, 2] <- cor(abs(i - temp_df$sched_dep_time_metric), temp_df$is_delay)}

filter(results_df, abs(cor_i) == max(abs(cor_i)))
```
...
```
Before continuing, I can provide the equation that estimates the probability of a flight being delayed using these five inputs:
...
test_00 <- flights %>%
  ...
  mutate(dep_time_object = hm(paste ## Next, convert departure time to a time object
                              (trunc(sched_dep_time/100) ## This line provides the hour
                                ,str_sub(sched_dep_time,-2,-1))) ## This line provides the minute
         ,sched_dep_time_metric = 100 * (trunc(sched_dep_time/100)) ## Next, convert scheduled departure time to metric time, which is something I made up to make the graph look better
...
You can also add a prediction using the multivariate model, but will not be able to graph it (in fewer than six dimensions)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
resi_err(46); resi_err(47); resi_err(50)
```

Looks like the bivariate model's the best. Let's plot all 3
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot() +
  ...
  ggtitle('Fly in the morning to lower your chance for delay') +
  scale_color_economist(labels = c('baseline p', 'pred p_bivar', 'observed p')
                     ,name = 'Prob. of delay given sched. dep. time') +
  theme_fivethirtyeight()
...
# Then put that file in this directory. After that, select the rest of this chunk and hit command-shift-C on a Mac or Ctrl-Shift-C on a Windows machine.
