...
flt_test_01 %>%
  ...
  unique() %>%
     ggplot(mapping = aes(x = sched_dep_time_metric, y = op_sched_dep, color = 'op_sched_dep')) +
     geom_point() +
  geom_smooth(method = 'loess', mapping = aes(color = 'sched_dep_time_metric')) +
  ggtitle('Likelihood of departure delay peaks around 8 p.m.') +
  scale_color_fivethirtyeight(name = 'Time diff. of scheduled departure and 8 p.m.') +
    theme_fivethirtyeight()
