...
```{r echo=FALSE, include=FALSE, warning=FALSE}
## Use the bivariate model  to predict probability for each scheduled departure time
test_d <- test_c %>%
  mutate(pred_bivar = 1/(1+exp(-(0.2746527 - 0.001169423 * diff_20h)))
         ,sched_dep_time_metric = round((sched_dep_time / 100 - trunc(sched_dep_time / 100)) ## I'm treating minutes as though they're base 10
                                        * 500/3 + 100 * (trunc(sched_dep_time / 100))))
```

```{r echo=FALSE, warning=FALSE}

ggplot(data = test_d, mapping = aes(x = sched_dep_time_metric, y = op_diff20, color = 'observed probability')) +
  geom_point(size = 1) +
  ...
  scale_color_fivethirtyeight(labels = c('observed probability', 'predicted probability')
...
flt_test_01 %>%
  ...
  unique() %>%
     ggplot(mapping = aes(x = sched_dep_time_metric, y = op_sched_dep, color = 'Observed probability by time')) +
     geom_point() +
  geom_smooth(method = 'loess', mapping = aes(color = 'Smoothed line of ')) +
  ggtitle('Likelihood of departure delay peaks around 8 p.m.') +
  scale_color_fivethirtyeight(name = 'Scheduled departure time') +
    theme_fivethirtyeight()
...
Now to mutate scheduled departure time:
```{r}
...
The package's documentation offers extensive metadata information: [nycflights documentation](https://cran.r-project.org/web/packages/nycflights13/nycflights13.pdf "nycflights docs")
