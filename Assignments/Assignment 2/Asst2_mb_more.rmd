...
install.packages('moments'); install.packages('ggalluvial')
...
flights %>%
  ...
  scale_color_fivethirtyeight(labels = c('Departures per half hour', 'Smoothed departures per half hour')
                              ,name = '') +
  ggtitle('Departure frequency is bimodal but departure delay is not')
```
...
flights %>%
  mutate(is_delay = case_when(dep_delay > 0 ~ 1
         ...
         + as.numeric(str_sub(as.character(sched_dep_time), -2, -1)) / 3 * 5
         ,diff_1935h = abs(as.numeric(dep_time_object - hm('19:35')) / 60)
         ,na_na_p = 1/(1+exp(-(0.279603750 - 0.001999519 * diff_1935h)))
         ,na_1_delay = if_else(is.na(dep_delay), 1, is_delay)
         ,na_1_p = 
         ,na_0_delay = if_else(is.na(dep_delay), 0, is_delay)
         ,na_0_p =3) %>%
