...
temp_00 <- flights %>%
  filter(!is.na(dep_delay)) %>%
  group_by(hour) %>%
  mutate(sched_dep_time_metric = round((sched_dep_time / 100 - trunc(sched_dep_time / 100)) * 500/3 + 100 * (trunc(sched_dep_time / 100)))
           ,is_delay = if_else(dep_delay > 0, 1, 0)
         ,op_hr = mean(is_delay, na.rm = T)
         ,dep_hr = n()) %>%
  ungroup() %>%
  group_by(dep_hr) %>%
  mutate(op_dephr = mean(is_delay, na.rm = T)) %>%
  ungroup() %>%
  group_by(sched_dep_time) %>%
  mutate(op_min = mean(is_delay, na.rm = T)
         ,dep_min = n()) %>%
  ungroup() %>%
  group_by(dep_min) %>%
  mutate(op_depmin = mean(is_delay, na.rm = T)) %>%
  ungroup() %>%
  mutate(dep_time_object = hm(paste
                              (trunc(sched_dep_time/100) ## This line provides the hour
                                ,str_sub(sched_dep_time,-2,-1))) ## This line provides the minute
  ...
  mutate(op_diff1935 = mean(is_delay, na.rm = T)) %>%
  ungroup() %>%
  ggplot(aes(dep_min, sched_dep_time)) +
  geom_point(aes(color = op_depmin)) +
  geom_smooth()
  ...
  # ggplot() +
  # geom_point(aes(sched_dep_time_metric, op_min, color = 'min'), size = 1) +
  # geom_point(aes(sched_dep_time_metric, op_depmin, color = 'dep min'), size = 1) +
  # geom_smooth(aes(sched_dep_time_metric, op_min, color = 'min smooth')) +
  # geom_smooth(aes(sched_dep_time_metric, op_depmin, color = 'dep min smooth')) +
  # geom_line(aes(sched_dep_time_metric, op_hr, color = 'hour', na.rm = T), size = 1.5) +
  # geom_line(aes(sched_dep_time_metric, op_dephr, color = 'dep hour', na.rm = T), size = 1.5)
