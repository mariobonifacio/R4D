...
1. Combine flight data with data on the planes used for each flight and the weather contemporaneous to each flight
...
flt_test <- flt_raw %>%
  ...
  mutate(diff_20h = if_else(sched_dep_time < 2000
                            ...
                            ,sched_dep_time - 2000)) %>%
  group_by(carrier) %>%
     mutate(op_carr = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(origin) %>%
     mutate(op_ogn = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(dest) %>%
     mutate(op_dest = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(type) %>%
     mutate(op_type = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(manufacturer) %>%
     mutate(op_mnf = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(model) %>%
     mutate(op_mdl = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(speed) %>%
     mutate(op_spd = mean(dep_delay, na.rm = T)) %>%
     ungroup() %>%
  group_by(engine) %>%
     mutate(op_eng = mean(dep_delay, na.rm = T)) %>%
     ungroup()
