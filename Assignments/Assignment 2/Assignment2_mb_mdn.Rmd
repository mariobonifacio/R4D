...
library(tidyverse); library(skimr); library(nycflights13); library(lubridate); library(ggthemes); library(reshape2)
```
```{r}
...
test_p <- flights[sample(nrow(flights),100),] %>%
...
test_p <- cbind(test_p,tibble(x_p = sample(0:1460, nrow(test_p),replace=T))) %>%
  mutate(y_p = 1/(1+exp(-(0.2746527 - 0.001169423 * x_p))))

test_p %>%
  ggplot(aes(x_p, y_p)) +
  geom_line() +
#  theme_economist() +
  geom_point(data = test_p, aes(x = diff_20h, y = is_delay),
    method = "glm", method.args = list(family = "binomial"), 
    se = FALSE) +
  geom_jitter(width = .01, height = .01)

test_p %>%
  mutate(meow = max(diff_20h))
