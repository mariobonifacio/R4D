...
```{r}
flights %>% filter(!is.na(dep_delay) == T, dep_delay > 15) %>% ggplot(mapping = aes(dep_delay)) + geom_histogram(binwidth = 10)

flights %>% filter(!is.na(dep_delay) == T, dep_delay > 30) %>% ggplot(mapping = aes(dep_delay)) + geom_histogram(binwidth = 10)

flights %>% filter(!is.na(dep_delay) == T, dep_delay > 60) %>% ggplot(mapping = aes(dep_delay)) + geom_histogram(binwidth = 10)
...
flights_17 %>%
  filter(!is.na(dep_delay) == T) %>%
  group_by(dep_delay) %>%
  summarise(p_delay = sum(dep_delay > 0) / length(dep_delay)
            ,diff_20h = )
